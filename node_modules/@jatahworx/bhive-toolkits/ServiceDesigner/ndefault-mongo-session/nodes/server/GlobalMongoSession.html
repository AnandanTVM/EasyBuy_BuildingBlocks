<script type="text/x-red" data-template-name="GlobalMongoSession">
<!-- Name of the node -->
<!-- proxy -->
...
<div id="autoremove-container" style="display:none">

    <!-- autoremove -->
    <div class="editor-form-row">
        <label for="node-input-autoremove-select">Auto Remove Session</label>
        <div class="http-element-container">
            <select id="node-input-autoremove" class="select-method">
            </select>
            <div class="editor-form-row field-wrapper-typedInput display-none" id="node-input-autoremove-map-container">
                <input type="text" id="node-input-autoremove-map" class="node-input-property typed-input-width"
                    style="width: 100% !important" />
                <input type="hidden" id="node-input-outputs" />
            </div>
        </div>
    </div>
    <div class="editor-form-row" id="node-input-autoremoveInterval-container" style="display:none">
        <input type="text" id="node-input-autoremoveInterval">
    </div>
</div>


<div class="ssd-edit-dialog-cookie-fields-accordion editor-form-row" style="margin-top: 1em;">
    <h3 class="pallete-category-title cursor-pointer ssd-optional-cookie-header">Cookie Options<img
            src="assets/pallete_icons/arrow_down.svg"></h3>
    ...
    registerNode({
        ...
        nodeDef: {
            ...
            defaults: {
                ...
                db: { type: "db-config", required: false, options: { type: 'mongodb', dbCategory: 'no-sql' } },
                store: { value: '' },
                autoremoveInterval: {
                    value: ''
                }
            ...
            label: function () {
                return this.name || 'Mongo Session'
            },
            labelStyle: function () {
            ...
            oneditprepare: function (SRD) {
                ...
                $('#node-input-autoremoveInterval').inputField({
                    placeholder: 'Auto remove interval',
                    value: node.autoremoveInterval || ''
				});
            ...
            oneditinit: function (SRD) {
                ...
                const autoremoveList = [
                    { value: 'native', displayValue: 'Native' },
                    { value: 'destroy', displayValue: 'Destroy' },
                ...
                const autoremoveDropDownElement = $('#node-input-autoremove').selectField({
                    optionsData: autoremoveList,
                    value: 'native'
                });
                ...
                function toggleAutoRemoveInterval(autoremove) {
                    if (autoremove != 'interval') {
                        $("#node-input-autoremoveInterval-container").css('display', 'none');
                    } else {
                        $("#node-input-autoremoveInterval-container").css('display', 'block');
                    }
                }

                $('#node-input-autoremove').on('change', (e) => {
                    toggleAutoRemoveInterval(e.target.value);
                })

				toggleAutoRemoveInterval(node.autoremove);
            },
            oneditsave: function (SRD) {
                const node = this;
            },
            docsLink: 'global-mongodb-session-node'
        }
    });
</script>