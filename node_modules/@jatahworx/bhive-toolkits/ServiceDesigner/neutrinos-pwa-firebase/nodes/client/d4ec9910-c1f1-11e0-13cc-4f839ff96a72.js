...
module.exports = class unsubscribe extends BaseComponent {
	constructor(constructorOptions) {
		super(
			constructorOptions.id,
			constructorOptions.name,
			'neutrinos_service_designer_' + 'unsubscribe',

			'd4ec9910-c1f1-11e0-13cc-4f839ff96a72'
		);
	...
	generateSnippet(serviceType, serviceClassTemplate) {
            ...
            return new Promise((resolve,reject)=>{
                try{
                    this.swPush.subscription.pipe(take(1)).subscribe((pushSubscription) => {
                        let result = false;
                        if (pushSubscription) {
                            pushSubscription.unsubscribe().then((res) => {
                            result = res;
        ...
        ${this.constructorOptions.resultMapping} =  await fcmUnsubscribe();
        `;
	}
