<script type="text/x-red" data-template-name="CallSSDApi">
<!-- Method -->
    <div id="method-input-wrp" class="http-container editor-form-row-margin-bottom-0">
    ...
    <!-- Method select -->
    <div id="node-select-method-wrp" class="editor-form-row">
        <label for="node-select-method"><span data-i18n="http-request.label.method"></span></label>
        <select id="node-select-method" style="width: 100%;">
        </select>
    </div>
    <!-- Service Select -->
    <div id="node-select-service-wrp" class="editor-form-row">
        <label for="node-select-service"><span data-i18n="http-request.label.service"></span></label>
        <select id="node-select-service" style="width: 100%;">
        </select>
    </div>
    <!-- Url -->
    <div id="node-select-url-wrp" class="editor-form-row">
        <label for="node-select-url"><span data-i18n="http-request.label.api"></span></label>
        <select id="node-select-url" style="width: 100%;">
        </select>
        <div class="error-msg-container"><span id="select-url-error" class="invalid-select">Select an API</div>
    </div>
    <!-- Path Params -->
    <div id="input-props-section" class="props-section editor-form-row">
        <label for="input-props-section"><span  data-i18n="http-request.label.routeparams"></span></label>
...
<script type="text/javascript">
    registerNode({
        ...
        nodeDef: {
            ...
            inputs: 1,
            outputs: 1,
            icon: "ndefault-all-nodes/call-ssd-node.svg",
            paletteLabel: function (def) { return 'Call Server API'; },
            label: function () {
                return this.name || "Call Server API";
            },
            labelStyle: function () {
            ...
            oneditprepare: function (SRD) {
                ...
                let routeParams = [];
                let startNwInputVar = [];
                serviceNames.push({value: 'All', displayValue: 'All'});
                $('#input-props-section').hide();
                
                function updateMethod() {
                if ($(`#node-select-url`).val() ) {
                    urloptions.forEach((item, index) => {
                        if (urloptions[index].value == $(`#node-select-url`).val()) {
                            this.method = urloptions[index].method;
                            methodSelectEle.val(urloptions[index].method);
                        }
                    });
                }
                }
                let serviceOptionsField = $(`#node-select-service`).selectField({
                    optionsData: [],
                    change: (val) => {
                        getUrl(nodeList, val, methodsOptionsField.selectField('selected'))
                        urlOptionsField.selectField('updateOptions', urloptions, true, true);
                        urlOptionsField.selectField('clearSelection');
                        $('#input-properties-content').children().remove();
                        $('#input-props-section').hide();
                        $('#select-url-error').show();
                    }
                });
                let methodsOptionsField = $(`#node-select-method`).selectField({
                    optionsData: httpMethods,
                    change: (val) => {
                        getUrl(nodeList, serviceOptionsField.selectField('selected'), val)
                        urlOptionsField.selectField('updateOptions', urloptions, true, true);
                        urlOptionsField.selectField('clearSelection');
                        $('#input-properties-content').children().remove();
                        $('#input-props-section').hide();
                ...
                let urlOptionsField = $(`#node-select-url`);
                SRD.utils.getNodesList(SRD.appName, 'server', 'HttpIn').subscribe(res => {
                  nodeList = res;
                  getUrl(res, 'All', 'All');
                 
                  urlOptionsField.selectField({
                    placeholder: 'Choose an API',
                    props: { required: true },
                    optionsData: urloptions,
                    value: this.url ? this.url : '' ,
					required: true,
                    ...
                    change: (url) => {
						...
						if(url) {
							getRoutesArray(url);
							fillVariables('input', getInputVariables(routeParams));
							updateMethod();
							$('#select-url-error').hide();						}
                    }
                });
                if (this.url) {
                    $('#select-url-error').hide();
                }          
                  serviceOptionsField.selectField('updateOptions', serviceNames, true, true);                  if (this.url) {
                    fillVariables('input', this.inputVariables);
                  }
                });

                function getUrl(nodesList, service, method) {
                    urloptions = [];
                    for (let i = 0; i < nodesList['data'].length; i++) {
                        if ( service === 'All' && method === 'All') {
                            urloptions.push({ value: nodesList['data'][i]['path'], displayValue: `${nodesList['data'][i]['method'].toUpperCase()} ${nodesList['data'][i]['path']}`, method: nodesList['data'][i]['method'] });
                        } else if ((service !== 'All' && method === 'All') && nodesList['data'][i]['serviceName'] === service) {
                            urloptions.push({ value: nodesList['data'][i]['path'], displayValue: `${nodesList['data'][i]['method'].toUpperCase()} ${nodesList['data'][i]['path']}`, method: nodesList['data'][i]['method'] });
                        } else if ((service === 'All' && method !== 'All') && nodesList['data'][i]['method'] === method) {
                            urloptions.push({ value: nodesList['data'][i]['path'], displayValue: `${nodesList['data'][i]['method'].toUpperCase()} ${nodesList['data'][i]['path']}`, method: nodesList['data'][i]['method'] });
                        } else if ((service !== 'All' && method !== 'All') && nodesList['data'][i]['method'] === method && nodesList['data'][i]['serviceName'] === service) {
                            urloptions.push({ value: nodesList['data'][i]['path'], displayValue: `${nodesList['data'][i]['method'].toUpperCase()} ${nodesList['data'][i]['path']}`, method: nodesList['data'][i]['method'] });
                        }
                        serviceFlag = true;
                        serviceNames.forEach((item, index) => {
                            if (serviceNames[index].value === nodesList['data'][i]['serviceName']) {
                                serviceFlag = false;
                            }
                        })
                        if (serviceFlag) {
                            serviceNames.push({ value: nodesList['data'][i]['serviceName'], displayValue: nodesList['data'][i]['serviceName'] });
                        }
                    }
                }
                ...
                function getInputVariables(inputVariables) {
                    ...
                    if (Array.isArray(inputVariables)) {
                        retVars = inputVariables.map((ele) => {
                            return { key: ele, value: '', mapVarType: 'input', valueType: 'bh', map: true }

                        });
                    }
                    return retVars;
                }
                function getRoutesArray (url) {
                    routeParams = [];
                    routeArr = SRD.utils.getPathArray(url);
                    for (let i = 0; i < routeArr.length; i++) {
                        if (typeof routeArr[i] == 'object') {
                            routeParams.push(routeArr[i].name);
                            $('#input-props-section').show();
                        } else {
                            $('#input-props-section').hide();
                        }
                    }
                ...
                /* Add route params properties section */
                ...
                function validateReturnType(value) {
                    isReturnTypeValid = validateTypedInput(value, 'invalid-returntype-mapping-error', 'node-input-returntype-button');
                ...
                function validateResultMapping(value) {
                    isResultMappingValid = validateTypedInput(value, 'invalid-resultmapping-mapping-error');
                ...
                $('#node-input-returntype-button').on('click', function () {
					...
					isReturnTypeValid = returnTypeTypedInputEle.typedInput('validate');
                ...
                if (this.headerMappingObj && this.headerMappingObj.hasOwnProperty('mapVarType')) {
                    ...
                    fillVariables('headers', this.headersObj);
                }
                ...
                if (this.paramMappingObj && this.paramMappingObj.hasOwnProperty('mapVarType')) {
                    ...
                    fillVariables('params', this.paramsObj);
                }
                repostionAddButton ()
                function repostionAddButton () {
                    ...
                    $('.add-prop-btn-wrp').each(function (i, elem) {
                        if (i==0) {
                            ...
                            .insertBefore($('.add-section .map-button-wrp')[i-1]);
                        }
                    });
            ...
            oneditsave: function (SRD) {
                ...
                /* Url*/
                let routeArr = SRD.utils.getPathArray($('#node-select-url').val());
                let ps = '';
                if (routeArr instanceof Array) {
                    for (let i = 0; i < routeArr.length; i++) {
                        if (typeof routeArr[i] == 'string') {
                            ps += `${routeArr[i].startsWith('/') ? '' : ''}${routeArr[i]}`;
                        }
                    }
                }
                this.urlValue = ps;
                node.url = $('#node-select-url').val();
                ...
                $(`#input-properties-content`).editableList('items').each(function (i) {
                    let row = $(this);
                    let inputObj = {};
                    inputObj.key = (row.find('.property-key').val());
